...
class ExternalResponse
{
    ...
    public function __construct($cURL_resorce, ExternalRequest $request)
    {
        ...
        if ($request->last_options_before_send['accept'] == 'text/yaml') {
            try {
                ...
                if (!is_array($this->output)) {
                    trigger_error("Response expected string in YAML format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
                }
            } catch (\Exception $e) {
                trigger_error("Response expected string in YAML format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
            }
        ...
        if ($request->last_options_before_send['accept'] == 'application/json') {
            try {
                ...
                if (!is_object($this->output)) {
                    trigger_error("Response expected string in JSON format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
                }
            } catch (\Exception $e) {
                trigger_error("Response expected string in JSON format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
            }
        ...
        if ($request->last_options_before_send['accept'] == 'application/xml') {
            try {
                ...
                if (!is_array($this->output)) {
                    trigger_error("Response expected string in XML format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
                }
            } catch (\Exception $e) {
                trigger_error("Response expected string in XML format. Conversion fail for request with URL = " . $request->final_url, E_USER_WARNING);
            }
    ...
    {
        if ($this->output) {
            if ($this->request->ioptions['accept'] === ExternalRequest::ACCEPT_JSON) {
                return @json_decode($this->output, true);
            } else if ($this->request->ioptions['accept'] === ExternalRequest::ACCEPT_XML) {
                $xml = @simplexml_load_string($this->output, "SimpleXMLElement", LIBXML_NOCDATA);
                $json = json_encode($xml);
                return @json_decode($json, true);
            } else if ($this->request->ioptions['accept'] === ExternalRequest::ACCEPT_YAML) {
