...
class HttpResponseHandler
{
    ...
    public static $mime_types = [
        ...
        'not allow' => 'application/pgp-encrypted',
    ...
    {
        if (!get_class($file) === 'Winged\File\File') {
            $file = new File($file, false);
        }
        if (get_class($file) === 'Winged\File\File') {
            if ($file->exists()) {
                if (array_key_exists($file->getExtension(), $this->ages)) {
                    header_remove();
                    header("Pragma: public");
                    header("Vary: Accept-Encoding");
                    header("Connection: Keep-Alive");
                    header('Access-Control-Allow-Origin: *');
                    if ($this->ages[$file->getExtension()]['charset']) {
                        header("Content-Type: " . HttpResponseHandler::$mime_types[$file->getExtension()] . "; charset=" . WingedConfig::$config->HTML_CHARSET);
                    } else {
